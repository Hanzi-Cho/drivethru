...
> 왼쪽 시뮬레이터에서 차량이 드라이브스루 건물 입구의 NFC 인식 범위에 들어가면, 오른쪽 AAOS 화면에서 드라이브스루 주문 앱이 열리고 주문과 결제 UI가 진행되며, 매장을 벗어나면 세션이 종료되는 데모 영상을 만든다.
...
3. 차량이 건물 입구 근처 NFC 범위와 겹친다.
4. 시뮬레이터가 앱 내부 `EntryDetected(storeId)` 이벤트를 보낸다.
5. AAOS 화면이 드라이브스루 주문 플로우로 전환된다.
6. 기어 상태가 `DRIVE`이면 간소화 메뉴를 보여준다.
7. `PARK` 전환 후 전체 메뉴가 열린다.
8. 사용자가 메뉴를 선택하고 주문 검토 화면으로 간다.
9. 결제 화면에서 지정 카드로 결제하는 UI를 보여준다.
10. 결제 완료 후 차량이 다시 이동한다.
11. 차량이 건물 영역을 벗어나면 주문 세션 종료 UI를 보여준다.
12. 앱은 종료 또는 홈 복귀 유사 화면으로 정리된다.
